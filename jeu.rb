class Personne
  ...
  def attaque(personne)
    ...
    if nom == "Jean-Michel Paladin"
      personne.subit_attaque(degats_bonus+Random.rand(50))
    else personne.subit_attaque(Random.rand(20))
    end
...
class Joueur < Personne
  ...
  def soin
    ...
    # - Gagner de la vie
    # - Affiche ce qu'il s'est passé
    pdv = Random.rand(100 - @points_de_vie)
    if points_de_vie < 100
      @points_de_vie = @points_de_vie + pdv
    end
    puts "#{nom} se soigne"
    puts "#{nom} récupère #{pdv} points de vie"
  end
  ...
  def ameliorer_degats
    ...
    # - Affiche ce qu'il s'est passé
    @degats_bonus = @degats_bonus + self.degats
    puts "#{nom} augmente sa puissance d'attaque"
  end
...
class Jeu
  ...
  def self.est_fini(joueur, monde)
    ...
    if joueur.en_vie == false
      puts "#{joueur.nom} a été vaincu"
      return true
    elsif monde.ennemis_en_vie.size == 0
      puts "#{joueur.nom} est victorieux (#{joueur.points_de_vie}/100pv)"
      return true
    else
      puts "#{joueur.nom} (#{joueur.points_de_vie}/100pv)"
    end
  end
...
class Monde
  ...
  def ennemis_en_vie
    ...
    ennemis.reject {|ennemi| ennemi.en_vie == false}
  end
...
100.times do |tour|
  ...
  if choix == 0
    ...
    if ennemi_a_attaquer.en_vie == false
      puts "L'ennemi est déjà mort."
      puts "Vous perdez un tour."
    else joueur.attaque(ennemi_a_attaquer)
    end
  end
  ...
  if monde.ennemis_en_vie.size > 0
    ...
    puts "TOUS LES ENNEMIS ONT ETE VAINCUS"
  end
